...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    const entry = await approvalService.rejectEntry(entryId, auth.user.id, body.reason);
    return ok({ entry });
  } catch (error) {
    return fail(
      {
        code: "REJECT_ENTRY_FAILED",
        message: error instanceof Error ? error.message : "Không thể từ chối entry",
        details: error,
      },
      400,
    );
